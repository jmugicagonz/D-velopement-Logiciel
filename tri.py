...
#On prend quelques libreries et aussi le code "vtk" qu'on a écrit pour pouvaoir transformer notre code en un fichier vtk et le
#répresenter avec PAraView.


class tri:
#Notre class "tri" va être une face d'un tétraèdre. On va metre trois points et elle va nous donner un triangle
#avec son vecteur normal correspondant. On trouve aussi les fonctions "Create_Mesh_And_Connectivity_List", "Create_Tri_and_Normal_Mesh_Connectivity_List"
#et "distance_to_a_point" lesquelles j'expliquerai ci-dessous.
    ...
    #appeler à la class "tri" va exécuter l'init et va créer un vecteur avec les troix sommets et aussi va garder le vecteur normal
    #correspondant.
    ...
    #avec cette fonction, on va amplier les listes des points et des conexions (pour répresenter les triangles en ParaView) avec
    #les sommets du triangle et ces connexions correspondantes.
        """(
    ...
    def Create_Tri_and_Normal_Mesh_Connectivity_List(self, List_Of_Point, 
                                                     Connectivity):
    #avec cette fonction, on va amplier les listes des points et des conexions (pour répresenter les triangles en ParaView) avec
    #les sommets du triangle et ces connexions correspondantes. Aussi elle va rajouter deux points du vecteur normal du triangle.
    #pour le vecteur normal, on va prendre le centre du triangle et en partant de celui-ci et en somant le vecteur normal,
    #on arrivera au deuxième point.
        """
        ...
        """
        List_Of_Point += [self.points[0],self.points[1],self.points[2],np.asarray([(self.points[0][0]+self.points[1][0]+self.points[2][0])/3,(self.points[0][1]+self.points[1][1]+self.points[2][1])/3,0.0]),np.asarray([(self.points[0][0]+self.points[1][0]+self.points[2][0])/3 + self.normal[0],(self.points[0][1]+self.points[1][1]+self.points[2][1])/3 + self.normal[1],self.normal[2]])]
        n = len(List_Of_Point)
    ...
    #Cette fonction je l'ai pris de l'internet et je l'ai modifié pour l'adapter à notre cas. On la donne un point et elle va
    #calculer la distance minimal du point au triangle. Elle va nous donner le valeur de cette distance mais aussi le point
    #du triangle correspondante à cette distance minimal avec p.
        ...
        # first we check if the face is pointing toward the point. Si le point est derrière le triangle, on donnera comme distance
        #la valeur infini. On verra dans les modules suivantes à qoui Ça sert.
        
        normal_side = False
        d_min = np.inf                   # Numpy Infinity
        
        if np.dot(self.normal,p-self.points[0]) < 0:
            return([normal_side,d_min])
        else:
            normal_side = True
            #dans notre fonction on va introduire la class "TRI" correspondante à notre triangle et le point p au lequel on veut
